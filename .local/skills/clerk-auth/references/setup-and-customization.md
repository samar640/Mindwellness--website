...
4. **Global CSS setup** (Tailwind v4 — the layer declaration must come before `@import 'tailwindcss'`):
...
5. **Disable lightningcss optimization in `vite.config.ts`** by passing `tailwindcss({ optimize: false })` to the `@tailwindcss/vite` plugin. Without this, nested `@layer` imports from `@clerk/themes/*.css` get reordered in prod builds and Clerk UI renders correctly in dev but broken in prod.
